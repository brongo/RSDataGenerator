...
{
    // Construct from .json
    PackageMapSpec::PackageMapSpec(fs::path packageMapSpecPath)
    {
        // open packageMapSpec.json filestream
        std::ifstream packageMapSpec_FStream = std::ifstream(packageMapSpecPath);

        // convert file contents to string
        std::stringstream packageMapSpec_SStream;
        packageMapSpec_SStream << packageMapSpec_FStream.rdbuf();
        std::string packageMapSpec_String = packageMapSpec_SStream.str();

        // parse packageMapSpec contents with jsonxx
        jsonxx::Object packageMapSpecJson;
        packageMapSpecJson.parse(packageMapSpec_String);
        ...
        // build list of map-file references
    ...
    // Returns list of .resources files in packagemapspec.json - *in order of load priority*
    std::vector<fs::path> PackageMapSpec::GetResourceFileList(const bool excludeMeta) const
    {
        std::vector<fs::path> resourceList;

        for (int i = 0; i < Files.size(); i++)
        {
            fs::path fileName = Files[i].Name;

            if (Files[i].Name == "meta.resources" && excludeMeta == true)
                continue;

            if (fileName.extension() == ".resources")
                resourceList.push_back(fileName.string());
        }
        return resourceList;
    }
}
