...
namespace RSDataParser
{
    ...
    *
    *   The class / structs here only describe the index section. None of the embedded file data is read in.
    *      For an example where embedded file data is read, see SAMUEL: https://github.com/brongo/SAMUEL
    */
    ...
    class ResourceFile
    {
        public:
            ...
            std::vector<std::string> GetAllStringEntries() const { return _stringEntries; }
            std::vector<ResourceFileEntry>& GetAllFileEntries() { return _FileEntries; }

            // Retrieves embedded file data for a specific entry
            std::vector<uint8_t> GetEmbeddedFileData(int entryNumber);

            // Constructor
            ResourceFile(const fs::path& filePath, const int loadPriority = 0);
        ...
        private:
            ...
            std::vector<FileDependency> _FileDependencies;  // (not implemented)  immediately after _stringEntries,  repeating 0x20 byte sequence
